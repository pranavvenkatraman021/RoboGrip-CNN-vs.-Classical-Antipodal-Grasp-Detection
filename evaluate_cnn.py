...
#preprocesses one image: crop to ROI, then resize
def preprocess(rgb, depth):
    ...
    return tensor, crop_w, crop_h, x_off, y_off

#converts model's output back to real image-coordinate corners
def output_to_corners(output, crop_w, crop_h, x_off, y_off):
    x_norm, y_norm, w_norm, h_norm, sin2t, cos2t = output

    x = (x_norm * crop_w) + x_off
    y = (y_norm * crop_h) + y_off
    w = w_norm * crop_w
    h = h_norm * crop_h
...
def evaluate_cnn(split_path="dataset_split.json", weights_path="grasp_model_best.pth"):
    ...
    for entry in test_ids:
        ...
        tensor, crop_w, crop_h, x_off, y_off = preprocess(rgb, depth)

        with torch.no_grad():
            output = model(tensor).squeeze(0).numpy()

        pred_corners = output_to_corners(output, crop_w, crop_h, x_off, y_off)
